...
def sliding_win_ds(data, targets, steps_in, steps_out=1, sliding_step=1):
    try:
        iter(steps_in)
    except TypeError:
        steps_in = range(steps_in)
    steps_in = np.array(steps_in)
    nsteps_in = max(steps_in)
    try:
        iter(steps_out)
    except TypeError:
        steps_out = range(nsteps_in+1, nsteps_in+1+steps_out)
    steps_out = np.array(steps_out)
    totwidth = max(max(steps_in), max(steps_out))+1
    nsamples = len(data) - totwidth + 1
